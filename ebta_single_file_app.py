...
def init_db():
    ...
    if cur.fetchone()["c"] == 0:
        seed = ALL_SUBJECTS
        cur.executemany("INSERT OR IGNORE INTO subjects(name,grade) VALUES(?,?)", seed) VALUES(?,?)", seed)
        # Ensure required subjects exist even if DB was previously seeded
        required_subjects = ALL_SUBJECTS
        cur.executemany("INSERT OR IGNORE INTO subjects(name,grade) VALUES(?,?)", required_subjects) VALUES(?,?)", required_subjects)
...
def home():
    ...
    required_subjects = [
        ...
        ("Mathematical Literacy", "G10"), ("Mathematical Literacy", "G11"),
    ]
    try:
